...
- **`HostControlService` (new, `Lattice.Core`)** owns one **serialized control lane per host**: a per-host FIFO tail-task chain (each submitted op awaits the previous op's completion task — submission order IS execution order by construction; deliberately not a `SemaphoreSlim`, whose blocked waiters have no documented FIFO order and could reorder Suspend-then-Resume into the wrong final state). Each op runs connect → auth (when a password is configured) → op RPC → dispose, on a fresh short-lived connection. Ops on different hosts run concurrently; ops on the same host queue on the lane (they are user-initiated and rare — no throughput concern).
...
- Phases: `Idle → LookupRequested → LookupPolling (poll counter in state) → AttachRequested → AttachPolling → Done of Result<string list, AttachError>` (success carries the daemon's attach messages for display).
- **What attach "success" means (and does not):** `error_num = 0` from the attach poll means the daemon accepted the request and created the project entry — `gstate.add_project`'s synchronous return (1.2). The daemon does **not** validate the authenticator at attach time; a bad account key (possible on the direct-key credentials path — the email/password path's authenticator comes freshly validated from `lookup_account`) surfaces only when the daemon's first scheduler RPC fails, as event-log messages. The UI therefore reports "attached" (never "verified"); post-attach reality flows in through the normal read path (the project row + event log), and the PR-J walkthrough includes a deliberate bad-key attach to pin this behavior.
...
| `AttachMachine` | machine | Exhaustive transition-table tests + FsCheck properties — safety and absorption, not liveness over arbitrary finite sequences (an arbitrary sequence may simply stop mid-flow): `step` never raises for any input sequence; per-stage poll commands never exceed the cap; `Done` is absorbing; `SendAttach`'s authenticator provenance (only from an `AuthenticatorKey` start or a zero-`error_num` lookup reply). Reaching `Done` is the interpreter's obligation (every command produces an input), exercised by the runner tests, not asserted over arbitrary machine-input sequences. |
