...
Contract:
- `static SingleInstanceGuard? TryAcquire(string lockPath, string pipeName)` → opens `lockPath` (`instance.lock` in the Lattice config dir; default derived from `LatticeConfig.DefaultPath`, parent dir created with the same user-only Unix mode as `LatticeConfig.Save`) with `FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None` and holds the handle; returns `null` on the sharing-violation `IOException`/`UnauthorizedAccessException` (another live instance holds it). No stale-lock handling needed: the OS releases the lock when the holder dies (Q3 rationale — a named `Mutex` was rejected because `Local\` is logon-session-scoped and fails the multi-terminal Unix case).
...
- Tests (red-first, no wall-clock settles, lock file in a temp dir): acquire→second acquire on the same path returns null; release→re-acquire succeeds; `SignalPrimary` before any listener returns false; listener + signal round-trip observed via a `TaskCompletionSource` (settle on the observed call, per repo test canon). All same-process — `FileShare.None` and pipes both enforce in-proc too.
