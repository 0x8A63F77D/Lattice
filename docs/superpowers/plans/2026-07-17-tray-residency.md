...
| F13 | `TrayIcon` exposes **no public signal** distinguishing a live tray icon from the silent no-op case (unsupported Linux DE): the platform impl is internal and the public surface is only the properties/`Clicked`/`Dispose` listed in F1. Runtime "is the tray actually there?" detection is impossible without out-of-band machinery (e.g. D-Bus probing). | ref-XML member scan of `T:Avalonia.Controls.TrayIcon` (F1); [TrayIcon.cs source](https://github.com/AvaloniaUI/Avalonia/blob/main/src/Avalonia.Controls/TrayIcon.cs) (impl field internal; Codex R4) |
...
|---|---|---|---|
| Close-to-tray | `UiState` (App-owned UI lifecycle pref) | `bool? ExitOnClose = null` — **null (and JSON-missing) = "platform default"**, resolved by the pure function `TrayResidencyDefaults.ExitOnCloseDefault(platform)`: Windows/macOS → `false` (close-to-tray ON, full tray support per F2), **Linux → `true` (exit-on-close)** — tray presence is not programmatically detectable (F13), so Linux residency is a deliberate user opt-in, never a silent default that can strand the app invisible. A user touching the toggle stores a concrete bool; older `ui-state.json` files (key missing) get the platform default with no migration | Settings toggle, label (localized): "Closing the window keeps Lattice running in the tray" — displayed toggle state is the *inverse* of the resolved value (VM inverts; contract in Task C2). On Linux the description string notes the StatusNotifierItem/AppIndicator requirement |
...
Contract:
- `static AcquireResult TryAcquire(string lockPath, string pipeName)` where `AcquireResult` is `Acquired(SingleInstanceGuard)` / `Contended` / `Unavailable` (small nested DU-style record hierarchy or enum + out param — implementer's choice, exhaustive switch at the call site). Opens `lockPath` (`instance.lock` in the Lattice config dir; default derived from `LatticeConfig.DefaultPath`, parent dir created with the same user-only Unix mode as `LatticeConfig.Save`) with `FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.None` and holds the handle. **On success, `TryAcquire` also starts the activation pipe listener BEFORE returning** (Codex R5): lock-acquire + listener-start are one atomic step as observed from outside, so a held lock with no answering listener can only mean a dying/foreign holder — never a primary that is merely slow to boot Avalonia. The listener's callback starts as a harmless no-op (a cold-starting primary is about to show its window anyway; a concurrent ping needs no extra action beyond being ANSWERED, which is what refutes/confirms the oracle) and is swapped later via `SetActivationCallback(Action)`. Residual gap: the few milliseconds between the lock `open` and the pipe `listen` inside `TryAcquire` involve no Avalonia/config work and sit far inside `SignalPrimary`'s 2 s connect timeout. Classification: success → `Acquired`; `IOException` on open (the lock-contention shape on both platforms) → `Contended`; `UnauthorizedAccessException` and path-shape errors (`DirectoryNotFoundException`, path-is-a-directory) → `Unavailable` — these mean the guard cannot operate, NOT that another instance exists. `Contended` is deliberately only *probable* contention — the pipe round-trip in A2 is the liveness oracle that confirms or refutes it (I-GUARD). No stale-lock handling needed: the OS releases the lock when the holder dies (Q3 rationale — a named `Mutex` was rejected because `Local\` is logon-session-scoped and fails the multi-terminal Unix case).
- Instance members: `SetActivationCallback(Action onActivate)` — swaps the listener's callback (initial value: no-op); the internal accept-loop is a background `NamedPipeServerStream`, one connection at a time, reads 1 byte, invokes the current callback (caller marshals to UI thread), loops; swallows per-connection `IOException`s (a dying secondary must not kill the listener); `Dispose()` cancels the loop and closes/releases the lock file handle.
...
- Tests (red-first, no wall-clock settles, lock file in a temp dir): acquire→second acquire on the same path returns `Contended`; release→re-acquire returns `Acquired`; lock path pointing at a DIRECTORY returns `Unavailable` (not `Contended` — the I-GUARD classification test); `SignalPrimary` before any acquire returns false; **`SignalPrimary` immediately after `TryAcquire` returns true with NO `SetActivationCallback` call made** (the R5 atomicity test — the listener must be live from `TryAcquire` itself); callback-swap + signal round-trip observed via a `TaskCompletionSource` (settle on the observed call, per repo test canon). All same-process — `FileShare.None` and pipes both enforce in-proc too.
...
- `Acquired` → normal primary launch; the pipe listener is ALREADY live (started inside `TryAcquire`, R5) with a no-op callback. The guard is stored in a static handed to `App`, which (desktop-guard block only) calls `SetActivationCallback` with a callback that (until PR C lands) posts `MainWindow.Show()`+`Activate()` via `Dispatcher.UIThread`; PR C swaps the callback body to `TrayResidencyController.ShowWindow`. Guard disposed in `desktop.Exit`.
...
**Files:** Modify `src/Lattice.App/Infrastructure/UiStateStore.cs` (`UiState` record: append `bool? ExitOnClose = null`), Create `src/Lattice.App/Infrastructure/TrayResidencyDefaults.cs`, Modify `src/Lattice.App/ViewModels/SettingsViewModel.cs`, `src/Lattice.App/Views/SettingsView.axaml`, `src/Lattice.App/Localization/Strings.resx` (keys in Part 4); Tests: existing `UiStateStore`/Settings headless test files + `TrayResidencyDefaultsTests.cs`

Contract: `TrayResidencyDefaults.ExitOnCloseDefault(platform)` is a tiny pure function (input: an OS-platform value the caller derives from `OperatingSystem.Is*`; output: `bool`) with a three-row transition-table test — Windows → `false`, macOS → `false`, Linux → `true` (F13 rationale in Part 4). Resolution helper `Resolve(bool? stored, platform) => stored ?? ExitOnCloseDefault(platform)`. VM property `CloseToTray` (inverted persistence — get `!Resolve(state.ExitOnClose, current)`, set stores a concrete bool via `UiStateStore.Update`, matching the Theme-toggle pattern in this VM); `FullSpeedHiddenPolling` property mirroring `PollingIntervalSeconds`'s `RegistryGuard.TryMutate` pattern. Two `ToggleSwitch` rows in SettingsView using the existing settings-row layout. Headless tests: toggle → persisted JSON asserts (stores concrete bool); pre-#92 ui-state.json fixture (key missing) loads as `null` and resolves to the current platform's default.
...
Contract = Part 5 in full, plus: `ExitOnClose` read live from `UiStateStore` at decide time and resolved through `TrayResidencyDefaults.Resolve` (not cached — respects the store's read-modify-write doctrine; the policy still receives a concrete bool); `NotifyExitingViaClose()` sets `ExitRequested` so the shutdown-driven second `Closing` (reason `ApplicationShutdown`) short-circuits identically on platforms that report `Undefined`; controller handed to `ShellWindow` via property (`_tray`), not DataContext (it's not a VM). `OnClosing` override = the exact snippet in Part 5. TrayIcon menu commands = plain `RelayCommand`s on the controller. Dispose order in `desktop.Exit`: tray first, then existing teardown chain. Headless: controller never constructed (guard) — assert existing suite still green; add a headless test that `ShellWindow` without a controller closes normally (regression: `_tray is null` path).
...
**Known platform gaps (explicit, not silent):** Windows and Linux tray legs ship code-complete but hardware-unverified (owner has macOS only — dev-environment reality). Close-interception is the Windows-critical path and is covered by the policy tests, but tray-icon visuals/left-click behavior on Windows (F3) and Linux appindicator presence (F2) need a hardware pass when available → file a follow-up issue at PR C merge time. The Linux fallback is **static, not detected** (Codex R4): Avalonia's `TrayIcon` offers no public signal separating a live icon from the silent no-op impl (F13), so a "detect failure → fall back" contract is unimplementable without new D-Bus machinery. Instead the close-to-tray *default* is platform-scoped (Part 4): exit-on-close on Linux, close-to-tray on Windows/macOS. A Linux user whose desktop has StatusNotifierItem/AppIndicator support opts in explicitly via the toggle — residency there is always a deliberate choice, so the app can never be silently stranded invisible by a default.
