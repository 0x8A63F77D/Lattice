...
- A bare `Dispatcher.UIThread.RunJobs()` is NOT a headless settle — always go through `HeadlessLayout.Layout(window)` / `Settle(root)` / `Pump()`. Two wall-clock traps hide behind it: a real layout pass (and therefore `SizeChanged`, `EffectiveViewportChanged`, virtualized container realization) only runs when Avalonia's headless 60 fps render `DispatcherTimer` happens to be DUE, so `Pump()` calls `ForceRenderTimerTick()` instead; and property transitions run off the REAL animation clock, so measure/arrange otherwise samples e.g. `SplitView`'s pane-open `DoubleTransition` at an arbitrary phase (one unchanged shell produced rail widths from 0 to 254, and at width 0 no rail container is realized at all). `Layout`/`Settle` detach transitions for the settle — an animated property falls back to its base, i.e. its target, value — and re-attach after, so wiring assertions still see them. This is what made the suite fail only on a cold first `dotnet test Lattice.sln`; pinned by HeadlessLayoutSettleTests.
...
- Headless test assemblies run STRICTLY SERIALLY (`[assembly: CollectionBehavior(DisableTestParallelization = true)]`, pinned by HeadlessSessionPolicyTests in both `Lattice.App.Tests` and `Lattice.VisualTests`). Avalonia's UI-thread identity is a process-global first-toucher-wins claim that every `AvaloniaObject` ctor makes via `Dispatcher.CurrentDispatcher`; `HeadlessUnitTestSession` (PerTest isolation) nulls and re-claims it around every `[AvaloniaFact]`, so a plain `[Fact]` body on a parallel xunit worker thread can steal it mid-rebuild and the session's next `VerifyAccess` throws (issue #169). Never add a headless test assembly without the attribute; never "fix" this family with sleeps, retries, or `AvaloniaTestIsolationLevel.PerAssembly` (which leaks app state across tests).
